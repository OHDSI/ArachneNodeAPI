var ko = {
    ...
    pureComputed: function (fu) {
        return function() { return fu() };
    }
}
...
function ComparativeCohortAnalysis(data) {
    ...
    if (data.treatmentCohortDefinition != null) {
        ...
        self.treatmentCohortDefinition = ko.observable(jsonCohortDefinition);
    } else {
    ...
    if (data.comparatorCohortDefinition != null) {
        ...
        self.comparatorCohortDefinition = ko.observable(jsonCohortDefinition);
    } else {
    ...
    if (data.outcomeCohortDefinition != null) {
        ...
        self.outcomeCohortDefinition = ko.observable(jsonCohortDefinition);
    } else {
    ...
    if (self.psExclusionId() > 0) {
        ...
        self.psExclusionConceptSet.push(conceptSetData);
    ...
    if (self.psInclusionId() > 0) {
        ...
        self.psInclusionConceptSet.push(conceptSetData);
    ...
    if (self.omExclusionId() > 0) {
        ...
        self.omExclusionConceptSet.push(conceptSetData);
    ...
    if (self.omInclusionId() > 0) {
        ...
        self.omInclusionConceptSet.push(conceptSetData);
    ...
    if (self.negativeControlId() > 0) {
        ...
        self.negativeControlConceptSet.push(conceptSetData);
...
function build(comparativeCohortAnalysisInfo) {
    var data = JSON.parse(comparativeCohortAnalysisInfo);
    var observableAnalysis = new ComparativeCohortAnalysis(data);

    var analysis = {};
    Object.keys(observableAnalysis).forEach(function(key) {
        if (typeof observableAnalysis[key] === "function") {
            analysis[key] = observableAnalysis[key]();
        } else {
            analysis[key] = observableAnalysis[key];
        }
    });

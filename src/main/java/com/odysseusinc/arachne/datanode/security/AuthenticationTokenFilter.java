...
public class AuthenticationTokenFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public void doFilter(
        ...
        try {
            if (authToken == null && ((HttpServletRequest) request).getCookies() != null) {
                Cookie cookie = WebUtils.getCookie((HttpServletRequest) request, this.tokenHeader);
                authToken = Optional.ofNullable(cookie).map(Cookie::getValue)
                        .orElseThrow(() -> new AuthException("user not regitered"));
            }
            if (authToken != null) {
